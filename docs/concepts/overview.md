...
| 组件 | 当前规模 | 作用 |
| ------ | ------ | ------ |
| **代理（Agents）** | 64 个代理文件 | 把复杂任务委派给专门角色，例如规划、审查、构建修复、文档更新 |
| **技能（Skills）** | 261 个技能目录 | 封装领域知识、工作流和可复用模式 |
| **命令（Commands）** | 84 个命令文件 | 作为 slash command 或命令入口调用工作流（上游也称之为 legacy command shims） |
...
3. **命令面显著扩大**
   - 已确认存在 84 个命令文件（上游也称之为 legacy command shims）。
